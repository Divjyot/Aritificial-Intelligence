...
public class MCTSSimulator {
	/** Record all position of Action 1 **/
	public int[] a1Record = new int[12];
	/** Record last move of Action 1 **/
	public int lastMove = 0;
	/** Record all action are performed by Simulator**/
	public int countingAction = 0;
	...
	/** change to true if want to write output file **/
	 ...
	 */
	public MCTSSimulator(ProblemSpec ps, String outputFile) {
		this.ps = ps;
	 ...
	 */
	public MCTSSimulator(String inputFile, String outputFile) throws IOException {
	...
	public State step(Action a) throws IllegalArgumentException {
		...
		if (steps > ps.getMaxT()) {
			...
			if(printOutput) outputSteps(false);
			return null;
		}
	...
	public State performA1() {
		...
		//record move distance to a1Record
		a1Record[moveDistance + 4] += 1;
...
//		System.out.println("Random move : " + move);
	 ...
	 * @return the next state
	 */
	public State performA2(Action a) {
	 ...
	 * @return the next state
	 */
	public State performA3(Action a) { return currentState.changeDriver(a.getDriverType()); }
	 ...
	 * @return the next state
	 */
	public State performA4(Action a) {
	 ...
	 * @return the next state
	 */
	public State performA5(Action a) {
	 ...
	 * @return the next state
	 */
	public State performA6(Action a) {
	 ...
	 * @return the next state
	 */
	public State performA7(Action a) {
...
//        steps += (stepsRequired - 1);
	 ...
	 *
	 *
	 * ADDED THIS TO MAKE IT COMPITBALE WITH OUR MCTSState . but is State classs is allowed to be modified then we can
	 * copy all the MCTSState class fucntions in State class itlsef ....
	 * @param ps
	 * @param currentState
	 */
	public MCTSSimulator(ProblemSpec ps, MCTSState currentState) {
		this.ps = ps;
	...
	/**
	 * Set a new state and return current state
	 *
	 * @return the current state
	 */
	public State setCurrentState(MCTSState state) {
		this.steps = state.getTotalSteps();
		this.currentState = state.copyState();
		stepRecord = new ArrayList<>();
		stepRecord.add(new Step(-1, currentState.copyState(), null));
		if (verbose) {
			System.out.println("Setting new state for simulator");
			System.out.println("Start " + currentState.toString());
		}
	...
	public int getStepLeft() {
		return this.ps.getMaxT() - this.getSteps();
	}

	public MCTSState getCurrentState() {
		return new MCTSState(currentState.copyState());
	}
